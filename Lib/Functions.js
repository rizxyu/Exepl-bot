...
conn.on('qr', async () =>{ console.log(this.color('[','white'), this.color('!','red'), this.color(']', 'white'), this.color('scan'))});
conn.on('credentials-updated', () => { console.log(this.chalk.bgGreen('login info update'))})
this.fs.existsSync(session) && conn.loadAuthInfo(session);
conn.on('connecting', async () => { console.log(
this.chalk.blueBright.italic('✅ Login information updated!')
)});
conn.on('open', async () => { console.log(
this.chalk.green.bold('✅ Login successful!'))}); 
...
async custom(imageUrl, top, bottom) {
 new Promise((resolve, reject) => {
   ...
   this.fetchBase64(`https://api.memegen.link/images/custom/${topText}/${bottomText}.png?background=${imageUrl}`, 'image/png')
